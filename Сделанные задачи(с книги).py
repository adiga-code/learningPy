...
#1)Создать класс, метод __init__ должен содержать два атрибута, создать экземпляр
class Restaurant():
...
#1)Создать класс. Применить метод __init__. Создать несколько экземпляров
class User():
...
#1)начать из упражнения 9.1. Добавить атрибут со значением по умолчанию, изменить 3-мя способами 
class Restaurant():
    def __init__(self , restaurant_name , cuisine_type):
        self.restaurant_name = restaurant_name
        self.cuisine_type = cuisine_type
...
#1)Из номера 9.3 создаются 2 метода, которые прибавляют и обнуляют атрибут
class User():
    def __init__(self , first_name , last_name):
        self.first_name = first_name
        self.last_name = last_name
...
user1.describe_user()
user1.greet_user()
#Изменение атрибута login_attempts
...
# Nom 9.9
#1)В программу про машины, добавить метод, который устанавливает емкость батареи
class Car():
...
# Nom 9.14.9.15
#1)Написать цикл, кот орый проверяет, насколько сложно выиграть в лотерее.
import random

print('Здравствуйте, выберите лотерейный билет')
symbols = [1, 2, 4, 3, 6, 7, 5, 9, 8, 0, 'A', 'B', 'C', 'D', 'E']
my_ticket = ['D', 'E', 0, 'A']
text_for_print = "Билет имеющий комбинацию", "".join('%s' %id for id in my_ticket), "является выигрышным!"   
print(text_for_print)
combinations = 0
while True:
    if my_ticket != random.sample(symbols, 4):
        combinations += 1
    if my_ticket == random.sample(symbols, 4):
        print("You win!")
        print("The winning combination was: {}".format(combinations))
        break
...
#Файлы и исключения
#чтение и работа с содержимыми документов

# Nom 10.1
#1)Вывести содержимое файла-документа 4-мя разными способами
